document.addEventListener('DOMContentLoaded', function() {
    ...
    const racas = {
        ...
        "Anão": {
            bonusFixos: { con: 2, sab: 1, des: -1 },
            extras: 0
        },
        "Dahllan": {
            bonusFixos: { sab: 2, des: 1, int: -1 }, // Corrigido de acordo com T20 JDA
            extras: 0
        }
        // Adicione mais raças aqui
    };
    let pontosExtras = 10; // Valor padrão para compra de atributos se nenhuma raça for selecionada
    let distribuidos = 0;
    let bloqueados = {};
    const atributos = ['for', 'des', 'con', 'int', 'sab', 'car'];


    // --- FUNÇÕES DE LÓGICA DE RAÇA E ATRIBUTOS (Vindo de atributos.js) ---
    ...
    function aplicarBonusRaca() {
        ...
        // Reseta os atributos para o padrão antes de aplicar os bônus
        distribuidos = 0;
        bloqueados = {};
        pontosExtras = 10; // Pontos padrão de nível 1
        
        atributos.forEach(attr => {
            document.getElementById(attr).value = 0;
        });

        if (racas[raca] && nivel === 1) {
            const { bonusFixos, extras } = racas[raca];
            // Para raças como Humano, os pontos extras são somados aos 10 iniciais.
            pontosExtras += extras;
    ...
    function atualizarSetas() {
        document.querySelectorAll('.atributo-container').forEach(container => {
            ...
            if (bloqueados[attr]) {
                ...
                // Lógica para habilitar/desabilitar seta de AUMENTAR
                if (distribuidos >= pontosExtras) { // Limite de 4 pontos comprados
                    up.style.opacity = '0.3';
                    up.style.pointerEvents = 'none';
                } else {
                    up.style.opacity = '1';
                ...
                // Lógica para habilitar/desabilitar seta de DIMINUIR
                if (valor <= -4) { // Não pode diminuir abaixo do valor base (-5)
                    down.style.opacity = '0.3';
                    down.style.pointerEvents = 'none';
                } else {
        ...
        document.getElementById('pontos-distribuicao').textContent = `Pontos: ${distribuidos}/${pontosExtras}`;
    }
...
function configurarSetasAtributos() {
    ...
    // --- FUNÇÕES DE CÁLCULO (Vindo de script.js) ---
    ...
    const calcularCarga = () => {
        ...
        let forca_peso = forca >= 1 ? forca * 2 : forca;
    ...
    const calcularTodasPericias = () => {
        ...
        Object.keys(PERICIAS).forEach(id => {
            ...
            const total = modAtributo + meioNivel + bonusTreino + outros + penalidadeAplicada;
    ...
    // --- FUNÇÕES DE INICIALIZAÇÃO E MANIPULAÇÃO DO DOM ---
    ...
    const inicializarAtributos = () => {
        // Cria um espaço para mostrar os pontos de distribuição
        const container = document.querySelector('.grid.grid-cols-6.gap-2.text-center');
        if (container) {
            const displayPontos = document.createElement('div');
            displayPontos.id = 'pontos-distribuicao-container';
            displayPontos.className = 'col-span-6 text-center font-medieval text-red-900 mt-2';
            displayPontos.innerHTML = `<span id="pontos-distribuicao">Pontos: 0/10</span>`;
            container.parentNode.insertBefore(displayPontos, container.nextSibling);
        }
    };
    ...
    const setupEventListeners = () => {
        ...
        // O listener das setas de atributo foi movido para configurarSetasAtributos() para evitar conflito.
        ...
        // Listeners para o sistema de raças
        document.getElementById('raca').addEventListener('change', aplicarBonusRaca);
        document.getElementById('nivel').addEventListener('input', aplicarBonusRaca); // Recalcula se o nível mudar
    };
    ...
    inicializarMagias();
    inicializarAtributos(); // Adicionado para criar o display de pontos
    preencherRacas(); // Preenche o select de raças
    configurarSetasAtributos(); // Configura a lógica de compra de pontos
    carregarDadosIniciais();
    recalcularTudo();
    setupEventListeners();
    aplicarBonusRaca(); // Chama uma vez para setar os valores iniciais
});